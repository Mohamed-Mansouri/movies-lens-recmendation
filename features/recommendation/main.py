...
Run: uvicorn features.recommendation.main:app --reload --port 8000
"""
import warnings
...
def _load():
    if not MODELS_DIR.exists() or not any(MODELS_DIR.glob("*.pkl")):
        raise HTTPException(503, "Models not found. Run 'python features/recommendation/train.py' first.")
...
def recommend(
    ...
    model:   str = Query("svd", description="svd | knn | xgboost"),
